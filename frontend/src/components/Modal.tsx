...
export const Modal = ({ isOpen, title, children, onClose, size = 'default' }: Props) => {
  if (!isOpen) return null

  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-stone-950/35 px-3 py-3 backdrop-blur-sm animate-fade-in sm:items-center sm:px-4 sm:py-8">
      <div
        className={[
          'max-h-[calc(100vh-24px)] w-full overflow-hidden rounded-2xl border border-stone-200 bg-[#fbfaf8] shadow-[0_30px_90px_rgba(28,25,23,0.22)] animate-pop-in sm:max-h-[calc(100vh-48px)]',
          size === 'wide' ? 'max-w-[800px]' : 'max-w-lg',
        ].join(' ')}
      >
